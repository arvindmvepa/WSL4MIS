...
def entropy_loss(p, C=2, gpus="cuda:0"):
    # p N*C*W*H*D
    y1 = -1*torch.sum(p*torch.log(p+1e-6), dim=1) / \
        torch.tensor(np.log(C), dtype=torch.float32).to(gpus)
    ent = torch.mean(y1)
    return ent
...
def entropy_loss_map(p, C=2, gpus="cuda:0"):
    ent = -1*torch.sum(p * torch.log(p + 1e-6), dim=1,
                       keepdim=True)/torch.tensor(np.log(C), dtype=torch.float32).to(gpus)
    return ent
...
class pDLoss(nn.Module):
    ...
    def forward(self, inputs, target, weight=None):
        ...
        print("debug, ignore_mask.size() = ", ignore_mask.size() )
